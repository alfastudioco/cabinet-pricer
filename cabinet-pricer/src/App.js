  ...
  const analyzeLayout = async () => {
    ...
    try {
      const blocks = [
        ...
        {
          type: "text",
          text: `You are a cabinet estimator. Read every dimension label printed on these elevation drawings exactly as shown. Do not estimate or approximate — only use numbers explicitly written on the drawings.

For each wall/elevation:
1. List every dimension label you can read (e.g. 25, 16.5, 24, 42)
2. Add them together for total LF — convert inches to feet (divide by 12)
3. Split: upperLF = totalLF * 0.5, lowerLF = totalLF * 0.5
4. Cost split: lowers = 60% of total, uppers = 40% of total
5. ${price ? `Grand total MUST equal exactly $${parseInt(price).toLocaleString()}. Work backwards from this number.` : "Estimate fair market price for: Grade:${grade}, Box:${box}, Door:${door}, Finish:${finish}, Hardware:${hardware}."}

Specs: Grade:${grade}, Box:${box}, Door:${door}, Finish:${finish}, Hardware:${hardware}

IMPORTANT: Use ONLY dimensions you can actually read on the drawing. Be consistent — same image must always produce same numbers.

Respond ONLY with valid JSON, no markdown:
{"walls":[{"name":"str","totalLF":0.0,"upperLF":0.0,"lowerLF":0.0,"upperCost":0,"lowerCost":0,"totalCost":0,"costPerLF":0,"dimensionsRead":["list every number you read from drawing"],"features":["str"]}],"totalLF":0.0,"totalUpperLF":0.0,"totalLowerLF":0.0,"totalUpperCost":0,"totalLowerCost":0,"grandTotal":0,"upperCostPerLF":0,"lowerCostPerLF":0,"blendedCostPerLF":0,"specs":["str"],"notes":"str"}`
        }
      ];
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 16, ...mono, fontSize: 12 }}>
              ...
              <tbody>
                {results.walls.map((w, i) => (
                  <tr key={i}>
                    <td style={cell(false)}>{w.name}<br/><span style={{fontSize:10,color:S.brown}}>{(w.dimensionsRead||[]).join(" + ")}</span></td>
